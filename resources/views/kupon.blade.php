      ...
      <div class="row g-3 g-md-4">
        @foreach($coupons as $coupon)
          @php
            ...
            $minOrder = data_get($coupon, 'cart_value') ?? data_get($coupon, 'min_order') ?? data_get($coupon, 'min_belanja');
          ...
          <div class="col-12 col-md-6 col-xl-4">
            <div class="card h-100 border-0 shadow-sm coupon-card">
              ...
              <div class="coupon-head d-flex align-items-center justify-content-between px-3 py-2">
                ...
                @if($isActive)
                  <!-- <span class="badge coupon-limited">Terbatas</span> -->
                @else
              ...
              <div class="card-body d-flex flex-column">
                {{-- Kiri: label KUPON + potongan | Kanan: KODE KUPON BESAR (mengganti 0%) --}}
                <div class="d-flex align-items-start justify-content-between mb-3">
                  ...
                  {{-- Kanan: tampilkan KODE KUPON besar, bukan 0% --}}
...
<style>
  .coupon-card {
    ...
    color: #fff;          /* teks default putih agar kontras */
  }
  ...
  .coupon-head {
    background: linear-gradient(90deg, #111 0%, #222 100%); /* header ikut gelap */
  ...
  .coupon-label {
    ...
    font-weight:700;
    letter-spacing:.06em;
    border:1.5px dashed #ffc107; /* dashed jadi kuning */
    padding:.4rem .7rem;
    border-radius:.6rem;
    background:#111;
    color:#ffc107;
  ...
  .coupon-summary .coupon-code-big {
    ...
    letter-spacing: .04em;
    background: #111;
    color: #ffc107;  /* kode kupon jadi kuning agar standout */
    padding: .35rem .6rem;
    border-radius: .75rem;
    display: inline-block;
    min-width: 120px;
  ...
  .coupon-card .coupon-ribbon {
    ...
    right: -40px;
    top: 12px;
    width: 140px;     /* diperkecil biar tidak melebihi bawah */
    height: 20px;
    background: repeating-linear-gradient(
      45deg,
      #ffc107 0 8px,   /* kuning solid */
...
<script>
  document.querySelectorAll('.copy-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      } catch (e) { alert('Gagal menyalin kode'); }
    });
  });
</script>
@endsection
